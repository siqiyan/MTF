...
#define PRE_SEED 10
#define POU false
#define PIX_MAPPER nullptr
#define DEBUG_MODE false

_MTF_BEGIN_NAMESPACE

//! value constructor
HISTParams::HISTParams(const AMParams *am_params,
...
HISTParams::HISTParams(const HISTParams *params) :
    AMParams(params),
    n_bins(N_BINS),
    pre_seed(PRE_SEED),
    partition_of_unity(POU),
    pix_mapper(PIX_MAPPER),
...
HISTDist::HISTDist(const string &_name, const int _n_bins,
...
HIST::HIST(const ParamType *hist_params, const int _n_channels) :
    AppearanceModel(hist_params, _n_channels),
    params(hist_params) {
	...
	name = "hist";
	log_fname = "log/mtf_hist_log.txt";
	time_fname = "log/mtf_hist_times.txt";
...
void HIST::initializePixVals(const Matrix2Xd& init_pts){
...
*		curr_pix_vals
*/
void HIST::updatePixVals(const Matrix2Xd& curr_pts){
	if(params.pix_mapper){
...
*/
void HIST::initializeSimilarity(){
...
*/
void HIST::updateSimilarity(bool prereq_only){
...
double HIST::getLikelihood() const{
	double d = (1.0 / f) - 1;
...
void  HIST::cmptInitHessian(MatrixXd &hessian, const MatrixXd &init_pix_jacobian){
...
void HIST::cmptSelfHessian(MatrixXd &self_hessian, const MatrixXd &curr_pix_jacobian){
...
* Computes::Description
*	hessian::first order Hessian of current HIST w.r.t. current pixel values
*/
void  HIST::cmptCurrHessian(MatrixXd &hessian, const MatrixXd &curr_pix_jacobian){
	int ssm_state_size = curr_pix_jacobian.cols();
	assert(hessian.rows() == ssm_state_size && hessian.cols() == ssm_state_size);
	assert(curr_pix_jacobian.rows() == patch_size);
...
void HIST::cmptInitHessian(MatrixXd &init_hessian, const MatrixXd &init_pix_jacobian,
...
*/
void HIST::cmptCurrHessian(MatrixXd &curr_hessian, const MatrixXd &curr_pix_jacobian,
...
void HIST::updateDistFeat(double* feat_addr){
...
double HISTDist::operator()(const double* hist1_mat_addr, const double* hist2_mat_addr,
